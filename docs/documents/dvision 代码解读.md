...
+ 实现文件1：`zed_camera.cpp`
![[1.1.png|666]]
+ 实现文件2：`pointcloud_process.cpp`
![[1.2.png|666]]
...
- 文件
    ...
    - 头文件（内外参数等算法结构体配置）：`dancer-vision/include/division/parameters.hpp`
...
- 头文件：`dancer-vision/include/dvidion/idetector.hpp`
...
- 主要类：CircleDetector（继承IDetector）
    ...
    - 关键方法（头文件）
        ...
        - `Init() override`：重写自IDetector类，用于初始化CircleDetectir，返回初始化是否成功
        - `Detect()`：从X形交叉点检测圆心
...
+ ==文件（新）==：`zed_obstacle_detector.cpp`
