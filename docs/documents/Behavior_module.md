...
1. 梳理从 launch 开始的启动流程，在哪里决定是否听裁判盒
2. 与裁判盒联调，理解裁判盒各指令的意义
3. 修改机器人的初始位置、开球后走到的位置
4. `motion_info.lower_board_connected` 会影响哪些行为
5. `teaminfo_outdated` 是不是太长了
6. 环境变量在哪里设置的
7. 如何让不同机器人的时钟同步？
8. `info.field_quality < 0.5`；球在多远的时候位置的置信度最高，是越近越好吗？
9. 避免机器人碰撞有两种方案：
   1) 如果自己是最近，就向队友声明是自己持球，队友走到合适的位置；如果自己不是最近（即有队友声明了，且自己判断一下队友的声明没问题），就走到合适的助攻点
   2) 两个 striker 选定一个作为决策者，只由他来判断谁持球谁助攻，并且向两个 striker（包括自己）发送“去某点并踢球”或“只是去某点”的指令。
   方案 2) 可以扩展到四个机器人，且将来用强化学习时一定是这种；而方案 1) 与当前的架构比较接近，短期内更容易实现。
   要实现方案 1)，有以下技术问题需要考虑：
   1. 持球者的决定属于覆盖式。开场时算出一个，此后每台机器人一直在循环地重算自己是不是最近。如果发现算出的结果和当前的 kicker 相矛盾，则发出新的声明。
